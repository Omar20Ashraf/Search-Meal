  ...
  <div class="max-w-[800px] mx-auto p-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2">
      <div>
        ...
        <ul>
          <template v-for="(el, ind) of new Array(20)">
            <li v-if="hasLetters(meal[`strIngredient${ind + 1}`])">
      ...
      <div>
        ...
        <ul>
          <template v-for="(el, ind) of new Array(20)">
            <li v-if="hasLetters(meal[`strMeasure${ind + 1}`])">
...
const meal = computed(() => store.getters.meal);

function getMealDetails() {
  store.dispatch('mealById', route.params.id)
}

function hasLetters(str) {
  const lettersOrNumbersRegex = /[A-Za-z0-9]/;
  return str && lettersOrNumbersRegex.test(str);
}
